...
parser.add_argument("--source", required=True, help="Source S3 bucket")
parser.add_argument("--frontend", default="uma-sandbox", help="Frontend project")
parser.add_argument("--destination", required=True, help="Destination S3 bucket")
parser.add_argument(
    "--check",
    action="store_true",
    default=False,
...
def sync(args):
    ...
    for f in src_files:
        ...
        dst_bucket.Object(path).copy_from(
            ...
            CacheControl=(
                f"max-age={args.max_age_static}, immutable"
                if path.startswith("static/") or path.startswith("_next/")
                else f"max-age=0, must-revalidate, s-maxage={args.s_maxage_root}, stale-if-error=86400"
            ),
